    ...
    <div x-data="productModalManager()">
                            ...
                            <select name="category_id"
                               ...
                               hover:border-indigo-400 transition duration-200">
                                ...
                                @foreach ($categories as $id => $name)
                                    <option value="{{ $id }}"
                                        {{ ($filters['category_id'] ?? '') == $id ? 'selected' : '' }}>{{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="brand_id"
                               ...
                               hover:border-indigo-400 transition duration-200">
                                ...
                                @foreach ($brands as $id => $name)
                                    <option value="{{ $id }}"
                                        {{ ($filters['brand_id'] ?? '') == $id ? 'selected' : '' }}>{{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="stock_status"
                               ...
                               hover:border-indigo-400 transition duration-200">
                                ...
                                <option value="in_stock"
                                    {{ ($filters['stock_status'] ?? '') == 'in_stock' ? 'selected' : '' }}>In Stock
                                </option>
                                <option value="low_stock"
                                    {{ ($filters['stock_status'] ?? '') == 'low_stock' ? 'selected' : '' }}>Low Stock
                                </option>
                    ...
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                Product</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">SKU
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                Category</th>
                        ...
                        @forelse($products as $product)
                            @php
                                ...
                                $viewData = [
                                    ...
                                    'stock' => \App\Helpers\NumberFormatter::format($stock),
                                    'image_url' => $product->image_url,
                                    'description' => $product->description,
                                    'status' => $product->is_active ? 'Active' : 'Inactive',
                                    'sale_price' => $product->sale_price,
                                    'purchase_price' => $product->latestPurchase
                                        ? $product->latestPurchase->cost_price
                                        : null,
                                ];
                                $openingStockData = [
                                    'id' => $product->id,
                            ...
                            <tr class="even:bg-gray-50 hover:bg-indigo-50 transition duration-150">
                                ...
                                <td class="px-6 py-4 text-right">
                                    @if ($product->latest_purchase_price !== null)
                                        <span
                                            class="text-sm text-gray-600">${{ \App\Helpers\NumberFormatter::format($product->latest_purchase_price) }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span
                                        class="text-sm font-semibold text-gray-800">${{ \App\Helpers\NumberFormatter::format($product->sale_price ?? 0) }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded-full">
                                            <i class="fas fa-times-circle"></i>
                                            Inactive
                                        </span>
                                    @endif
        ...
        <div x-show="showViewModal" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showViewModal" @click.outside="closeModals()"
                    class="inline-block w-full max-w-lg p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
                    ...
                    <div class="flex gap-4 mb-4">
                        <div class="h-24 w-24 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
                            <img x-show="viewProductData.image_url" :src="viewProductData.image_url"
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-500">Barcode: <span
                                    x-text="viewProductData.barcode || '-'"></span></p>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="p-3 bg-gray-50 rounded-lg">
                            ...
                            <p class="font-medium text-gray-800" x-text="formatNumber(viewProductData.stock)"></p>
                        </div>
                        <div class="p-3 bg-gray-50 rounded-lg">
                            <p class="text-xs text-gray-500">Sale Price</p>
                            <p class="font-medium text-gray-800">$<span
                                    x-text="formatNumber(viewProductData.sale_price)"></span></p>
                        </div>
                        <div class="p-3 bg-gray-50 rounded-lg">
                            <p class="text-xs text-gray-500">Status</p>
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full"
                                :class="viewProductData.status === 'Active' ? 'bg-green-100 text-green-700' :
                    ...
                    <div class="mt-4">
                        ...
                        <p class="text-sm text-gray-700 mt-1"
                            x-text="viewProductData.description || 'No description available'"></p>
                    </div>
        ...
        <!-- Stock Modal -->
        <div x-show="showStockModal" x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" @click="closeModals()"></div>
                <div
                    class="inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-800">Opening Stock</h3>
                        <button @click="closeModals()" class="text-gray-400 hover:text-gray-600 transition">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <form method="POST" :action="stockFormAction">
                        @csrf
                        @method('PATCH')
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Product</label>
                                <p class="mt-1 text-gray-900 font-semibold" x-text="stockProductName"></p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Current Stock</label>
                                <p class="mt-1 text-gray-900">
                                    <span x-text="formatNumber(stockCurrentStock)"></span><span x-text="stockUnitShortName ? ' ' + stockUnitShortName : ''"></span>
                                </p>
                            </div>
                            <div>
                                <label for="stock_quantity" class="block text-sm font-medium text-gray-700">Quantity <span
                                        class="text-red-500">*</span></label>
                                <input type="number" name="quantity" id="stock_quantity" min="1"
                                    class="mt-1 w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg text-gray-900 text-sm
                                   focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-400 focus:outline-none"
                                    required>
                            </div>
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mb-1" x-show="stockPriceHint">Defaulted from last purchase
                                </p>
                                <div class="relative">
                                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500"></span>
                                    <input type="number" name="purchase_price" id="purchase_price" step="0.01"
                                        min="0" x-model="stockPurchasePrice"
                            ...
                            <div>
                                <label for="stock_type" class="block text-sm font-medium text-gray-700">Type <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label for="stock_note" class="block text-sm font-medium text-gray-700">Note</label>
                                <textarea name="note" id="stock_note" rows="2"
                                    class="mt-1 w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg text-gray-900 text-sm
                                   focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-400 focus:outline-none"
        ...
        <div x-show="showWholesaleModal" x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" @click="closeModals()"></div>
                <div x-show="showWholesaleModal" @click.outside="closeModals()"
                    class="inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <form method="POST" action="" id="wholesaleForm">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label for="wholesale_price" class="block text-sm font-semibold text-gray-700">Wholesale
        ...
        <div x-show="showDeleteModal" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" @click="closeModals()"></div>
                <div x-show="showDeleteModal" @click.outside="closeModals()"
                    class="inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="py-4">
                        ...
                        <p class="text-center text-gray-500 text-sm mt-2"><span x-text="deleteProductName"
        ...
        <!-- Group Price Modal -->
        <div x-show="showGroupPriceModal" x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" @click="closeModals()"></div>
                <div x-show="showGroupPriceModal" @click.outside="closeModals()"
                    class="inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="group_price" class="block text-sm font-medium text-gray-700">Price <span
                                    class="text-red-500">*</span></label>
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span>
        ...
        function productModalManager() {
            return {
                ...
                async openStockModal(product) {
                    this.stockFormAction = '/products/' + product.id + '/opening-stock';
                    this.stockProductName = product.name;
                    this.stockCurrentStock = product.current_stock;
                    this.stockUnitShortName = product.unit_short_name || '';
