...
<div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
    <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
        <form method="POST" action="{{ route('roles.store') }}" x-data="roleManager()">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="guard_name" id="guard_name" value="{{ old('guard_name', 'web') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('guard_name')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                            ...
                            <div x-show="openGroups['{{ $group }}']" x-transition class="grid grid-cols-2 gap-2">
                                ...
                                <label class="flex items-center p-2 rounded hover:bg-gray-50 cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" @change="updateSelected" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
            ...
            <div class="mt-6 flex justify-end">
                ...
                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300 transform hover:scale-105">Create Role</button>
            </div>
        </form>
    </div>
</div>
