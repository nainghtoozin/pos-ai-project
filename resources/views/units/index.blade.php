        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Manage product units</p>
                </div>
            </div>
            @can('unit.create')
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Unit Name</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Short Name</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="even:bg-gray-50 hover:bg-indigo-50 transition duration-150">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-600">{{ $unit->short_name }}</td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            @can('unit.edit')
                            <button @click="editModal({{ json_encode(['id' => $unit->id, 'name' => $unit->name, 'short_name' => $unit->short_name, 'status' => $unit->status]) }})"
                            ...
                            @endcan
                            @can('unit.delete')
                            <button @click="deleteModal({{ json_encode(['id' => $unit->id, 'name' => $unit->name]) }})"
                ...
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center gap-2">
                            <i class="fas fa-balance-scale text-gray-300 text-4xl"></i>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="showCreateModal" @click.outside="closeModals()" class="inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Create New Unit</h3>
                <button @click="closeModals()" class="text-gray-400 hover:text-gray-600 transition">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <form method="POST" action="{{ route('units.store') }}" id="createForm">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label for="create_name" class="block text-sm font-semibold text-gray-700">Unit Name <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label for="create_short_name" class="block text-sm font-semibold text-gray-700">Short Name <span class="text-red-500">*</span></label>
                        <input type="text" name="short_name" id="create_short_name" 
                            class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg text-gray-800 text-sm
                                   focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-400 focus:outline-none
                                   hover:border-indigo-400 transition duration-200"
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="showEditModal" @click.outside="closeModals()" class="inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
            ...
            <form method="POST" action="" id="editForm">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="edit_name" class="block text-sm font-semibold text-gray-700">Unit Name <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label for="edit_short_name" class="block text-sm font-semibold text-gray-700">Short Name <span class="text-red-500">*</span></label>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="showDeleteModal" @click.outside="closeModals()" class="inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-xl shadow-xl transform transition-all sm:align-middle">
            ...
            <div class="py-4">
                ...
                <p class="text-center text-gray-600">Are you sure you want to delete this unit?</p>
                <p class="text-center text-gray-500 text-sm mt-2"><span x-text="deleteUnitName" class="font-semibold"></span></p>
            </div>
...
function unitModalManager() {
    return {
        ...
        editModal(unit) {
            ...
            document.getElementById('editForm').action = '/units/' + unit.id;
            this.showEditModal = true;
        },

        deleteModal(unit) {
            this.deleteUnitName = unit.name;
            document.getElementById('deleteForm').action = '/units/' + unit.id;
