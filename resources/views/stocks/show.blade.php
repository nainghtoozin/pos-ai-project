...
@section('content')
    <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        ...
                        <span
                            class="font-semibold text-emerald-600">+{{ \App\Helpers\NumberFormatter::format($stockSummary['opening']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Purchase</span>
                        <span
                            class="font-semibold text-emerald-600">+{{ \App\Helpers\NumberFormatter::format($stockSummary['purchase']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Sale Return</span>
                        <span
                            class="font-semibold text-emerald-600">+{{ \App\Helpers\NumberFormatter::format($stockSummary['sale_return']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Transfer In</span>
                        <span
                            class="font-semibold text-emerald-600">+{{ \App\Helpers\NumberFormatter::format($stockSummary['transfer_in']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Adjustment Increase</span>
                        <span
                            class="font-semibold text-emerald-600">+{{ \App\Helpers\NumberFormatter::format($stockSummary['adjustment_in']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center pt-3">
                        <span class="font-semibold text-gray-700">Total Stock In</span>
                        <span
                            class="font-bold text-emerald-600 text-lg">+{{ \App\Helpers\NumberFormatter::format($stockSummary['stock_in']) }}
                            {{ $unitShortName }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        ...
                        <span
                            class="font-semibold text-rose-600">-{{ \App\Helpers\NumberFormatter::format($stockSummary['sale']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Purchase Return</span>
                        <span
                            class="font-semibold text-rose-600">-{{ \App\Helpers\NumberFormatter::format($stockSummary['purchase_return']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Transfer Out</span>
                        <span
                            class="font-semibold text-rose-600">-{{ \App\Helpers\NumberFormatter::format($stockSummary['transfer_out']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Adjustment Decrease</span>
                        <span
                            class="font-semibold text-rose-600">-{{ \App\Helpers\NumberFormatter::format($stockSummary['adjustment_out']) }}
                            {{ $unitShortName }}</span>
                    </div>
                    <div class="flex justify-between items-center pt-3">
                        <span class="font-semibold text-gray-700">Total Stock Out</span>
                        <span
                            class="font-bold text-rose-600 text-lg">-{{ \App\Helpers\NumberFormatter::format($stockSummary['stock_out']) }}
                            {{ $unitShortName }}</span>
                    </div>
                </div>
            </div>
        ...
        <div
            class="bg-gradient-to-br from-indigo-600 via-indigo-500 to-blue-500 rounded-2xl shadow-lg p-8 text-black relative overflow-hidden">
            <div class="absolute top-0 right-0 w-32 h-32 bg-white opacity-5 rounded-full -mr-16 -mt-16"></div>
            <div class="absolute bottom-0 left-0 w-24 h-24 bg-white opacity-5 rounded-full -ml-12 -mb-12"></div>
            <div class="relative text-center">
                <p class="text-indigo-100 text-sm uppercase tracking-wider mb-2">Current Stock Balance</p>
            ...
            <form method="GET" action="{{ route('stocks.show', $product) }}" class="flex flex-wrap gap-4 items-end">
                ...
                @if (request()->has('date_from') || request()->has('date_to') || request()->has('search'))
                    <a href="{{ route('stocks.show', $product) }}"
                        class="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 font-medium rounded-lg transition">
                        Clear
                    </a>
                @endif
            </form>
                    ...
                    <thead class="bg-gray-50 border-b border-gray-100 sticky top-0">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                                Reference No</th>
                            <th
                                class="px-6 py-3 text-right text-xs font-semibold text-emerald-600 uppercase tracking-wider">
                                In Qty</th>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-rose-600 uppercase tracking-wider">
                                Out Qty</th>
                        ...
                        @forelse($movements as $movement)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $sourceType = $movement->source_type ?? ($movement->type ?? 'purchase');
                                        $color = \App\Models\StockMovement::getSourceTypeColor($sourceType);
                                        $label = \App\Models\StockMovement::getSourceTypeLabel($sourceType);
                                    @endphp
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 bg-{{ $color }}-100 text-{{ $color }}-700 text-xs font-medium rounded-full">
                                        {{ $label }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    @if (isset($movement->in_qty) && $movement->in_qty > 0)
                                        <span
                                            class="font-semibold text-emerald-600">+{{ \App\Helpers\NumberFormatter::format($movement->in_qty) }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-right">
                                    @if (isset($movement->out_qty) && $movement->out_qty > 0)
                                        <span
                                            class="font-semibold text-rose-600">-{{ \App\Helpers\NumberFormatter::format($movement->out_qty) }}</span>
                                    @else
                                        <span class="text-gray-300">-</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <span
                                        class="font-bold {{ $movement->running_balance >= 0 ? 'text-gray-800' : 'text-rose-600' }}">
                                        {{ \App\Helpers\NumberFormatter::format($movement->running_balance) }}
                                        {{ $unitShortName }}
                                    </span>
                                </td>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <p class="text-gray-400 text-sm">Stock movements will appear here after purchases
                                            or sales</p>
                                    </div>
                                </td>
                            </tr>
